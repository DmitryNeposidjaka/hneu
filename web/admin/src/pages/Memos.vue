<template>
    <div>
        <el-dialog
                ...
                width="60%">
            <create-memo style="padding: 0px 100px 0px 50px" v-on:memoCreated="memoCreated"
        ...
        <el-dialog
                ...
                width="60%">
            <edit-memo ref="edit-user-form" style="padding: 0px 100px 0px 50px" v-on:memoEdited="memoEdited"
                          v-bind:memo="userOnEdit" v-on:userEditClose="editUserVisible = false"
        ...
        <el-dialog
                ...
                width="40%">
            <delete-memo ref="edit-user-form" style="padding: 0px 100px 0px 50px" v-on:memoDeleted="memoDeleted"
                            v-bind:memo="userOnDelete"
                            v-on:userDeleteClose="deleteUserVisible = false"></delete-memo>
        </el-dialog>
        <el-row>
            <el-col :span="4">
            ...
            <div style="text-align: right; width: 70%; float: right;">
                <div style="padding: 0px 50px">
                    <el-button type="primary" icon="el-icon-plus" circle title="Add memo"
                ...
                <template slot-scope="props">
                    <el-card class="box-card">
                        <div slot="header" class="clearfix">
                            <span class="memo-card-title">{{ props.row.title }}</span>
                            <span class="memo-card-date">{{ props.row.created_at }}</span>
                        </div>
                        <el-row>
                            <el-col style="padding-left: 20px; margin: 20px 0px; color: #1d68a7">
                                <a :href="defaultUrl+'/'+props.row.lang+'/memo/'+props.row.link">{{defaultUrl+'/'+props.row.lang+'/memo/'+props.row.link}}</a>
                            </el-col>
                        </el-row>
                        <el-row>
...
<style>
    ...
    .memo-card-image-place {
        text-align: center;
    }

    .memo-card-image-place img {
        width: 200px;
...
<script>
    import CreateMemo from '../components/memos/CreateMemo';
    import DeleteMemo from '../components/memos/DeleteMemo';
    import EditMemo from '../components/memos/EditMemo';

    export default {
        components: {CreateMemo, EditMemo, DeleteMemo},
        data() {
            return {
                languages: [
                    {
        ...
        methods: {
            ...
            memoDeleted(user) {
                ...
                this.$message({
                    ...
                    message: 'The memo <strong>' + user.title + '</strong> has been deleted!'
                });
                this.getData();
            },
            clearRefresh() {
